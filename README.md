...
- `openai_responses` 出站的 provider-specific capability 配置
  - `responses_previous_response_id`
  - `responses_builtin_tools`
  - `responses_tool_result_status_error`
  - `responses_assistant_history_native`
- 关键链路单元测试、回归测试与流程测试
...
这些字段的含义是：
- `responses_previous_response_id`：是否支持基于 `previous_response_id` 的续轮
- `responses_builtin_tools`：是否支持 Responses 原生 builtin tools，例如 `web_search`
- `responses_tool_result_status_error`：是否支持 `function_call_output.status=error`
- `responses_assistant_history_native`：是否支持直接保留 assistant history，而不需要重写成 user 文本

当上游只部分兼容时，Syrogo 会优先读取这组显式声明；如果没有配置，再使用 provider 内置的默认兼容画像。
