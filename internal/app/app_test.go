...
func baseDualProtocolConfig() config.Config {
	cfg := baseConfig()
	cfg.Clients = append(cfg.Clients, config.ClientSpec{Name: "anthropic-key", Token: "anthropic-token"})
	cfg.Inbounds = append(cfg.Inbounds, config.InboundSpec{
		Name:     "anthropic-entry",
		Protocol: "anthropic_messages",
		Path:     "/v1/messages",
		Clients:  []config.ClientBindingSpec{{Ref: "anthropic-key", Tag: "office"}},
	})
...
func TestNewExposesConfiguredClientQuotaStats(t *testing.T) {
	cfg := baseConfig()
	cfg.Clients[0].Quota = config.ClientQuotaConfig{
		Enabled: true,
...
func TestNewInjectsClientQuotaTrackerIntoDispatcher(t *testing.T) {
	cfg := baseConfig()
	cfg.Clients[0].Quota = config.ClientQuotaConfig{
		Enabled: true,
		Windows: []config.QuotaWindowConfig{{Name: "cost", Type: "cost", Duration: config.DurationValue("1h"), MaxCostUSD: json.Number("1")}},
	}
	app, err := New(cfg)
	if err != nil {
		t.Fatalf("New() error = %v", err)
	...
	body := []byte(`{"model":"unpriced-model","messages":[{"role":"user","content":"hello"}]}`)
	w := httptest.NewRecorder()
...
func TestNewStreamsResponsesJSONContentPartFromResponsesOutbound(t *testing.T) {
	...
	cfg.Clients = append(cfg.Clients, config.ClientSpec{Name: "responses-key", Token: "responses-token"})
	cfg.Inbounds = append(cfg.Inbounds, config.InboundSpec{
		Name:     "responses-entry",
		Protocol: "openai_responses",
		Path:     "/v1/responses",
		Clients:  []config.ClientBindingSpec{{Ref: "responses-key", Tag: "responses-office"}},
	})
...
func TestNewEstimatesUsageForOpenAIChatOutboundWhenProviderOmitsUsage(t *testing.T) {
	...
	cfg.Routing.Rules[0].ToTags = []string{"openai-primary"}
	cfg.Routing.Rules[0].TargetModel = "gpt-5.4"
	...
	body, err := json.Marshal(map[string]any{"model": "gpt-4o-mini", "messages": []map[string]any{{"role": "user", "content": "hello"}}})
	if err != nil {
		t.Fatalf("json.Marshal() error = %v", err)
	}
	...
	got := w.Body.String()
	if !strings.Contains(got, `"prompt_tokens":`) || !strings.Contains(got, `"completion_tokens":`) || !strings.Contains(got, `"total_tokens":`) {
		t.Fatalf("body = %q, want estimated openai usage object", got)
	}
}

func TestNewEstimatesUsageForAnthropicOutboundWhenProviderOmitsUsage(t *testing.T) {
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		_ = json.NewEncoder(w).Encode(map[string]any{
			"id":          "msg_est_app_1",
			"type":        "message",
	...
	body, err := json.Marshal(map[string]any{
		...
		"messages": []map[string]any{{
			"role":    "user",
			"content": []map[string]any{{"type": "text", "text": "只回复 pong"}},
		}},
	})
	if err != nil {
		t.Fatalf("json.Marshal() error = %v", err)
	...
	w := httptest.NewRecorder()
	app.Server.Listeners()[0].Handler.ServeHTTP(w, authorizedRequest(http.MethodPost, "/v1/messages", "anthropic-token", body))
	if w.Code != http.StatusOK {
		t.Fatalf("route status = %d, want 200, body = %s", w.Code, w.Body.String())
	}
	got := w.Body.String()
	if !strings.Contains(got, `"input_tokens":`) || !strings.Contains(got, `"output_tokens":`) {
		t.Fatalf("body = %q, want estimated anthropic usage object", got)
	}
}
...
func TestNewBridgesOpenAIChatInboundToOpenAIResponsesOutbound(t *testing.T) {
	...
	cfg := baseConfig()
	cfg.Inbounds[0].Clients[0].Tag = "openai-to-responses"
	cfg.Routing.Rules = []config.RoutingRule{{
		Name:        "openai-to-responses-route",
		FromTags:    []string{"openai-to-responses"},
		ToTags:      []string{"responses-primary"},
		Strategy:    "failover",
		TargetModel: "gpt-5.4",
	}}
	...
	got := w.Body.String()
	if !strings.Contains(got, `"id":"resp_chat_to_responses_1"`) || !strings.Contains(got, `"object":"chat.completion"`) || !strings.Contains(got, `"content":"pong"`) {
		t.Fatalf("body = %q, want openai chat response bridged from responses outbound", got)
	}
	if !strings.Contains(got, `"prompt_tokens":18`) || !strings.Contains(got, `"completion_tokens":13`) || !strings.Contains(got, `"total_tokens":31`) {
		t.Fatalf("body = %q, want openai usage mapped from responses outbound", got)
	}
}

func TestNewBridgesOpenAIChatInboundToOpenAIChatOutbound(t *testing.T) {
	var upstreamBody struct {
		Model    string `json:"model"`
		Messages []struct {
			Role    string `json:"role"`
	...
	got := w.Body.String()
	if !strings.Contains(got, `"object":"chat.completion"`) || !strings.Contains(got, `"content":"pong"`) {
		t.Fatalf("body = %q, want openai chat completion response with pong", got)
	}
	if !strings.Contains(got, `"prompt_tokens":18`) || !strings.Contains(got, `"completion_tokens":13`) || !strings.Contains(got, `"total_tokens":31`) {
...
func TestNewRejectsResponsesBuiltinToolsWhenCapabilityDisabled(t *testing.T) {
	cfg := baseDualProtocolConfig()
	cfg.Clients = append(cfg.Clients, config.ClientSpec{Name: "responses-to-responses-key", Token: "responses-token"})
	cfg.Inbounds = append(cfg.Inbounds, config.InboundSpec{
		Name:     "responses-entry",
		Protocol: "openai_responses",
		Path:     "/v1/responses",
		Clients:  []config.ClientBindingSpec{{Ref: "responses-to-responses-key", Tag: "responses-to-responses"}},
	})
	cfg.Listeners[0].Inbounds = append(cfg.Listeners[0].Inbounds, "responses-entry")
	cfg.Routing.Rules = []config.RoutingRule{{
...
func TestNewBridgesResponsesInboundToOpenAIChatOutbound(t *testing.T) {
	...
	cfg := baseDualProtocolConfig()
	cfg.Clients = append(cfg.Clients, config.ClientSpec{Name: "responses-to-chat-key", Token: "responses-token"})
	cfg.Inbounds = append(cfg.Inbounds, config.InboundSpec{
		Name:     "responses-entry",
		Protocol: "openai_responses",
		Path:     "/v1/responses",
		Clients:  []config.ClientBindingSpec{{Ref: "responses-to-chat-key", Tag: "responses-to-chat"}},
	})
	cfg.Listeners[0].Inbounds = append(cfg.Listeners[0].Inbounds, "responses-entry")
	cfg.Routing.Rules = []config.RoutingRule{{
...
func TestNewBridgesOpenAIChatInboundToAnthropicOutbound(t *testing.T) {
	...
	cfg := baseConfig()
	cfg.Inbounds[0].Clients[0].Tag = "openai-to-anthropic"
	cfg.Routing.Rules = []config.RoutingRule{{
		Name:        "openai-to-anthropic-route",
		FromTags:    []string{"openai-to-anthropic"},
		ToTags:      []string{"anthropic-primary"},
		Strategy:    "failover",
		TargetModel: "claude-sonnet-4-6",
	}}
	...
	got := w.Body.String()
	if !strings.Contains(got, `"object":"chat.completion"`) || !strings.Contains(got, `"content":"pong"`) {
		t.Fatalf("body = %q, want openai chat response bridged from anthropic outbound", got)
	}
	if !strings.Contains(got, `"prompt_tokens":18`) || !strings.Contains(got, `"completion_tokens":13`) || !strings.Contains(got, `"total_tokens":31`) {
		t.Fatalf("body = %q, want openai usage mapped from anthropic outbound", got)
	}
}

func TestNewBridgesAnthropicEmptyToolResultContentToAnthropicOutbound(t *testing.T) {
	var requests []struct {
		Model    string                     `json:"model"`
		Messages []anthropicUpstreamMessage `json:"messages"`
	}
	...
	if toolResult.Type != "tool_result" || toolResult.ToolUseID != "toolu_empty" {
...
func TestNewBridgesResponsesInboundToAnthropicOutbound(t *testing.T) {
	...
	cfg := baseDualProtocolConfig()
	cfg.Clients = append(cfg.Clients, config.ClientSpec{Name: "responses-to-anthropic-key", Token: "responses-token"})
	cfg.Inbounds = append(cfg.Inbounds, config.InboundSpec{
		Name:     "responses-entry",
		Protocol: "openai_responses",
		Path:     "/v1/responses",
		Clients:  []config.ClientBindingSpec{{Ref: "responses-to-anthropic-key", Tag: "responses-to-anthropic"}},
	})
	cfg.Listeners[0].Inbounds = append(cfg.Listeners[0].Inbounds, "responses-entry")
	cfg.Routing.Rules = []config.RoutingRule{{
...
func TestNewUsageStatsAggregatesByKeyAndProvider(t *testing.T) {
	...
	cfg := baseConfig()
	cfg.Clients = []config.ClientSpec{{Name: "office-key", Token: "token-a"}, {Name: "office-key-2", Token: "token-b"}}
	cfg.Inbounds[0].Clients = []config.ClientBindingSpec{{Ref: "office-key", Tag: "office"}, {Ref: "office-key-2", Tag: "office"}}
	cfg.Outbounds = []config.OutboundSpec{{
		Name:      "openai",
		Protocol:  "openai_chat",
		Endpoint:  upstream.URL + "/v1",
...
func TestNewBindsAllCoreInboundsForMinimalSmoke(t *testing.T) {
	...
	listeners[0].Handler.ServeHTTP(responsesResp, authorizedRequest(http.MethodPost, "/v1/responses", "client-token", responsesBody))
